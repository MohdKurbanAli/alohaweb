...
class License_Page extends Base_Page {
	...
	public function get_name() {
		return esc_attr__( 'License', 'zemez-dashboard' );
	}
	...
	public function enqueue_license_assets() {
		...
		wp_enqueue_script(
			...
			zemez_dashboard()->plugin_url( 'assets/js/settings.js' ),
			array( 'jquery' ),
		...
		wp_localize_script( 'jet-theme-core-settings', 'JetSettingsData', array(
			'messages' => array(
				'empty' => esc_html__( 'Please enter your license key.', 'zemez-dashboard' ),
			),
	...
	public function activate_license( $data ) {
		...
		if ( true === zemez_dashboard()->api->acitvate_license( $license ) ) {
			...
			set_transient(
				...
				zemez_dashboard()->api->get_activation_error(),
				HOUR_IN_SECONDS / 3
			);
	...
	public function deactivate_license() {
		...
		if ( true === zemez_dashboard()->api->deacitvate_license( $license ) ) {
			$redirect = $this->get_current_page_link();
			set_site_transient( 'update_plugins', null );
		} else {
	...
	public function render_page() {
		...
		ob_start();
		include zemez_dashboard()->get_template( 'dashboard/license/' . $action . '-license-control.php' );
