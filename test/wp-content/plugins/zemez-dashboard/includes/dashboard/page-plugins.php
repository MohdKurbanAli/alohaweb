...
class Plugins_Page extends Base_Page {
	...
	public function get_name() {
		return esc_attr__( 'Plugins', 'zemez-dashboard' );
	}
	...
	public function install_plugin() {
		...
		\Zemez_Dashboard\Utils::install_plugin( $plugin );
	}
	...
	public function activate_plugin() {
		$plugin = isset( $_REQUEST['plugin'] ) ? esc_attr( $_REQUEST['plugin'] ) : false;
		\Zemez_Dashboard\Utils::activate_plugin( $plugin );
	}
	...
	public function update_plugin() {
		...
		if ( empty( $_REQUEST['plugin'] ) ) {
			wp_send_json_error( array(
				...
				'errorMessage' => __( 'No plugin specified.', 'zemez-dashboard' ),
			) );
		}
		...
		if ( ! current_user_can( 'update_plugins' ) || 0 !== validate_file( $plugin ) ) {
			$status['errorMessage'] = __( 'Sorry, you are not allowed to update plugins for this site.', 'zemez-dashboard' );
		...
		if ( $plugin_data['Version'] ) {
			...
			$status['oldVersion'] = sprintf( __( 'Version %s', 'zemez-dashboard' ), $plugin_data['Version'] );
		}
		...
		if ( is_wp_error( $skin->result ) ) {
			...
			if ( true === $plugin_update_data ) {
				$status['errorMessage'] = __( 'Plugin update failed.', 'zemez-dashboard' );
				wp_send_json_error( $status );
			}
	...
	public function enqueue_plugins_assets() {
		...
		wp_enqueue_script(
			...
			zemez_dashboard()->plugin_url( 'assets/js/plugins.js' ),
			array( 'jquery' ),
	...
	public function render_actions() {
		...
		printf(
			...
			__( 'Check for updates', 'zemez-dashboard' ),
			add_query_arg(
				array(
		...
		printf(
			...
			__( 'Check for new plugins', 'zemez-dashboard' ),
			add_query_arg(
				array(
					'jet_action' => $this->get_slug(),
	...
	public function render_core_block() {
		...
		$logo           = zemez_dashboard()->plugin_url( 'assets/img/logo.svg' );
		$slug           = ZEMEZ_DASHBOARD_PLUGIN_NAME;
		...
		$docs           = zemez_dashboard()->config->get( 'documentation' );
		$license        = $this->manager->get( 'license' );
		$license_key    = \Zemez_Dashboard\Utils::get_license();
		$logo          .= '?ver=' . zemez_dashboard()->get_version();

		include zemez_dashboard()->get_template( 'dashboard/plugins/core.php' );
	...
	public function render_jet_plugins_block() {
		...
		if ( ! $plugins ) {
			$api_url  = zemez_dashboard()->api->api_url( 'plugins' );
			$response = wp_remote_get( $api_url, zemez_dashboard()->api->request_args() );
		...
		foreach ( $plugins as $plugin ) {
			...
			include zemez_dashboard()->get_template( 'dashboard/plugins/plugin.php' );
	...
	public function install_plugin_link( $slug = '' ) {
		...
		if ( empty( $this->has_license ) ) {
			...
			printf(
				...
				__( 'Activate License to Install', 'zemez-dashboard' )
			);
