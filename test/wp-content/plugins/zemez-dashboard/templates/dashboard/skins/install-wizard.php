...
if ( array_key_exists( $slug, $all_plugins ) ) {
	...
	$label  = esc_html__( 'Activate', 'zemez-dashboard' );
} else {
	$action = 'install-wizard';
	$label  = esc_html__( 'Install Plugin', 'zemez-dashboard' );
}

?>
<div class="jet-install-wizard">
	<div class="jet-install-wizard__content"><?php
		_e( 'You need to install and activate <b>JetPluginsWizard</b> to manage skins', 'zemez-dashboard' );
	?></div>
